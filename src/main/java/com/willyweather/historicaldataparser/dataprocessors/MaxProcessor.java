...
/**
 * This class process MAX values of historical weather data
 */
public class MaxProcessor extends AbstractValueProcessor {
	...
	public void process(List<HistoricalWeatherData> weatherDataList) {
		...
		String joinedStnValueString = weatherDataList
										.stream()
										.map((HistoricalWeatherData data) -> this.valueRoundUp(this.parseDoubleValue(data.getMax())))
										.map(Object::toString)
										.collect(Collectors.joining(","));
		
		System.out.println("\nMAX: "+joinedStnValueString+"\n");
