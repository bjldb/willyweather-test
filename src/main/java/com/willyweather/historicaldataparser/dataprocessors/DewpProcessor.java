...
/**
 * This class process DEWP values of historical weather data
 */
public class DewpProcessor extends AbstractValueProcessor {
	...
	public void process(List<HistoricalWeatherData> weatherDataList) {
		...
		String joinedStnValueString = weatherDataList
										.stream()
										.map((HistoricalWeatherData data) -> this.valueRoundUp(this.parseDoubleValue(data.getDewp())))
										.map(Object::toString)
										.collect(Collectors.joining(","));
		
		System.out.println("\nDEWP: "+joinedStnValueString+"\n");
