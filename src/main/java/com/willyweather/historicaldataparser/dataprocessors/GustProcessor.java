...
/**
 * This class process GUST values of historical weather data
 */
public class GustProcessor extends AbstractValueProcessor {
	...
	public void process(List<HistoricalWeatherData> weatherDataList) {
		...
		String joinedStnValueString = weatherDataList
										.stream()
										.map((HistoricalWeatherData data) -> this.valueRoundUp(this.parseDoubleValue(data.getGust())))
										.map(Object::toString)
										.collect(Collectors.joining(","));
		
		System.out.println("\nGUST: "+joinedStnValueString+"\n");
