...
/**
 * This class process SLP values of historical weather data
 */
public class SlpProcessor extends AbstractValueProcessor {
	...
	public void process(List<HistoricalWeatherData> weatherDataList) {
		...
		String joinedStnValueString = weatherDataList
										.stream()
										.map((HistoricalWeatherData data) -> this.valueRoundUp(this.parseDoubleValue(data.getSlp())))
										.map(Object::toString)
										.collect(Collectors.joining(","));
		
		System.out.println("\nSLP: "+joinedStnValueString+"\n");
