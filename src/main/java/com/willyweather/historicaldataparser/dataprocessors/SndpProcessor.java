...
/**
 * This class process SNDP values of historical weather data
 */
public class SndpProcessor extends AbstractValueProcessor {
	...
	public void process(List<HistoricalWeatherData> weatherDataList) {
		...
		String joinedStnValueString = weatherDataList
										.stream()
										.map((HistoricalWeatherData data) -> this.valueRoundUp(this.parseDoubleValue(data.getSndp())))
										.map(Object::toString)
										.collect(Collectors.joining(","));
		
		System.out.println("\nSNDP: "+joinedStnValueString+"\n");
